...
\begin{itemize}
  \item Approach
  \begin{itemize}
    \item a task is associated with each output data partition
    \item the task performs the computation with its part of the output data
  \end{itemize}
  \item Requirements
  \begin{itemize}
    \item output contains many separable values
  ...
  \begin{itemize}
    \item \textbf{the task assigned a particular data item is responsible for all computation associated with it}
    \item the rule implies that all computations that use the output data assigned to a task \(t\) are performed by the task \(t\)
  \end{itemize}
  \item Examples
  \begin{itemize}
    \item filter operations in image processing
    \item matrix multiplication
  \end{itemize}
  \item Example
  \begin{itemize}
    \item multiplying two \(n x n\) matrices \(A\) and \(B\) to yield matrix \(C\)
  \end{itemize}
  \item Approach
  \begin{itemize}
    \item the output matrix \(C\) can be partitioned into \(n x n\) tasks as follows
    \[
      \left(
      \begin{matrix}
        A_{1,1} & A_{1,2}\\
...
\begin{itemize}
  ...
  \item Requirements
  \begin{itemize}
    \item input contains many separable values
    \item \textbf{output data partitioning is not possible}, because
    \begin{itemize}
      \item output contains only a few values
      \item the individual output elements cannot be efficiently determined in isolation
    \end{itemize}
  \end{itemize}
  \item Owner Computes Rule
  \begin{itemize}
  ...
  \item Examples
  \begin{itemize}
    \item finding minimum, maximum, median, sum of values
  \end{itemize}
\end{itemize}
