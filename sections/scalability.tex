...
\begin{itemize}
  \item Recall
  \begin{itemize}
    \item \(W\) is the asymptotic number of operations associated with the best serial algorithm to solve the problem: \(W = \Theta(T_S)\)
    \item overhead \(T_O(W,p)\) is a function of both problem size \(W\) and the number of processing elements \(p\)
    \item cost-optimal parallel systems have an efficiency of \(\Theta(1)\)
  \end{itemize}
  \item Definition
  \begin{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item isoefficiency function: \(f(p) = \Theta(p log(p))\)
  \end{itemize}
  \item Interpretation
  \begin{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \begin{align*}
      ...
      W = K^4p^3  
    \end{align*}
    \(\rightarrow\) the larger of these two asymptotic rates (\(p^{3/2}\) and \(p^{3}\)) determines the isoefficiency: \(f(p) = \Theta(p^3)\) 
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item a parallel system is cost-optimal iff \(\text{Cost} = p \cdot T_P = \Theta(W)\)
  \end{itemize}
  \item Lower Bound
  \begin{itemize}
    \item[\-]
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item \(W\) must increase at least as fast as \(\Theta(p)\) to maintain fixed efficiency: \(f(p) = \Omega(p)\)
    \begin{align*}
      p &= \mathcal{O}(W)\\
      W &= \Omega(p)\\
      W &= f(p) = \Omega(p)
...
\begin{itemize}
  ...
  \begin{itemize}
    \item isoefficiency due to concurrency is optimal only if \(C(W) = \Theta(W)\)
    \item if \(C(W) < \Theta(W)\) then the isoefficiency due to concurrency is worse than \(\Theta(p)\)\\
...
\begin{itemize}
  \item Problem
  \begin{itemize}
    \item solving a system of \(n\) equations in \(n\) variables by using Gaussian elimination \(W = \Theta(n^3)\)
  \end{itemize}
  \item Approach
  \begin{itemize}
    \item the \(n\) variables must be eliminated sequentially, one after the other, and eliminating each variable requires \(\Theta(n^2)\) computations\\
    \(\rightarrow\) at most \(\Theta(n^2)\) processing elements can be kept busy at any time\\
    \(\rightarrow\) \(p = \mathcal{O}(W^{2/3})\)
  \end{itemize}
  \item Degree of Concurrency
  \begin{itemize}
    \item \(C(W) = \mathcal{O}(W^{2/3}) < \Theta(W)\)
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item[\-] \(p = \mathcal{O}(C(W)) = \mathcal{O}(W^{3/2}) \rightarrow p^{3/2} = \mathcal{O}(W) \rightarrow \Omega(p^{3/2}) = W\)
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \begin{itemize}
    \item \(\frac{d}{dp}T_p = 0\), let \(p_0\) be the value of \(p\) as determined by this equation:\\
    \(T_p^{\text{min}} \begin{cases}
  ...
  \begin{itemize}
    ...
    \item if \(W = \Omega(f(p))\), then \(f(p) = \mathcal{O}(W)\), then \(p = \mathcal{O}(f^{-1}(W))\)
    \item for cost-optimal systems the overhead is \(\mathcal{O}(1)\): \(T_P = \Theta(W/p)\)\\
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item[\-] \(p = f^{-1}(W) = n/(K log(p)) \approx \Theta(n/log(n))\)  
  \end{itemize}
  \item Maximum number of processing elements to solve the problem cost-optimally
  \item[\-] \(p^{\text{cost\_opt}} \approx \Theta(n/log(n))\)
...
\begin{itemize}
  ...
  \begin{tabularx}{0.5\linewidth}{lXXXX}
    ...
    \hline
  \end{tabularx}
  \item The answer depends on your optimization criterion or criteria:
  \begin{itemize}
    \item If your main object is speed, then probably choose A1, because it has constant processing time, which is favorable if instance sizes \(n\) are large. However, constant time does not mean small, so it could be possible that for very small \(n\) A3 or A4 or even A2 could be faster.
    \item For the full speed of A1 you need \(n^2\) processing elements. This might be a huge number and there are good reasons that your parallel system doesn't offer that much processing elements. In such a case it would be advisable to choose an algorithm which makes use of a reasonable number of processing elements and with a good efficiency, e.g. A2 and A4. If \(\sqrt{n}\) is not already too much for the number of processing elements, then A4 is the better choice than A2, because of the smaller runtime.
  \end{itemize}
\end{itemize}