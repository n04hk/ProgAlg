...
\begin{itemize}
    ...
    \begin{itemize}
        \item best algorithm has to read the entire input: \(T_S = \mathcal{O}(n)\)
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \(T_P = \mathcal{O}(log(n))\)
    \end{itemize}
    \item Speedup and Efficiency
    \begin{itemize}
        \item \(S = \mathcal{O}(\frac{n}{log(n)})\)
        \item \(E = \frac{S}{p} = \frac{S}{n} = \mathcal{O}(\frac{1}{log(n)})\)
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \(E = \frac{T_S}{\text{Cost}} = \frac{\mathcal{O}(n)}{\mathcal{O}(n log(n))} = \mathcal{O}(\frac{1}{log(n)}) \neq \mathcal{O}(1)\)
        \item since the serial runtime of this operation is \(\mathcal{O}(n)\), the algorithm is \textbf{not cost-optimal}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item[\-] \(\rightarrow T_P = \mathcal{O}(\frac{n}{p} log(p)) + \mathcal{O}(\frac{n}{p}) = \mathcal{O}(\frac{n}{p} log(p)) \)
        \item[\-] \(\rightarrow \text{Cost} = \mathcal{O}(n log(p))\), it is not cost-optimal by a factor of \(log(p)\)\\
    ...
    \begin{itemize}
        \item each processing element locally adds its \(n/p\) numbers in time \(\mathcal{O}(n/p)\)
        \item the \(p\) partial sums on \(p\) processing elements can be added in time \(log(p)\)
        \item[\-] \(\rightarrow T_P = \mathcal{O}(\frac{n}{p} + log(p))\)
        \item[\-] \(\rightarrow \text{Cost} = \mathcal{O}(n + p log(p))\)
        \item[\-] \(S = \frac{n}{\frac{n}{p} + log(p)}\)
        \item[\-] \(E = \frac{1}{1 + p \frac{log(p)}{n}}\)
        \item this is cost-optimal, if \(p log(p) = \mathcal{O}(n)\) or if \(n = \mathcal{O}(p log(p))\)    
        \begin{multicols}{2}
